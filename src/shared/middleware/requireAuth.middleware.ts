 ...
 * Fails silently and cleanly calls next() when no token or an invalid token is provided,
 * without throwing an unhandled exception or setting an invalid user context.
 * If a valid active session is provided, populates req.user.
 */
export const optionalAuth = async (
  ...
  try {
    ...
    // Fail silently on any unexpected error or token parsing issue; proceed as unauthenticated
    next();
  }
};
